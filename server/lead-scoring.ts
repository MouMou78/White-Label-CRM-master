...
export function calculateFitScore(
  ...
  for (const rule of FIT_SCORING_RULES) {
    ...
    // Check if rule applies to account or person
    if (rule.ruleId.includes("industry") || rule.ruleId.includes("employee")) {
      matches = account ? rule.check(account) : false;
    } else {
      matches = rule.check(person);
    }
 ...
 * Uses exponential decay with 21-day half-life
 */
export function calculateIntentScore(
  events: Array<{ type: string; timestamp: Date }>
): { score: number; reasons: string[] } {
  const now = Date.now();
  const HALF_LIFE_MS = 21 * 24 * 60 * 60 * 1000; // 21 days
  const MIN_DECAY = 0.1;
  ...
  for (const [eventType, count] of sortedEvents) {
    reasons.push(`${eventType} (${count}x)`);
  }
...
/**
 * Calculate combined score (60% fit, 40% intent)
 */
